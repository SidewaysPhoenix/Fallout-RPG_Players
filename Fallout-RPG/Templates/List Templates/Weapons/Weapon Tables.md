...
const excludedFolders = ["Automatron Melee Weapons", "Unique Items"]; // Folders to exclude
const priorityFolders = ["LegendaryWeapons", "PrototypeWeapons"]; // Higher priority folders (only used if randomizeSelection = true)
...
items.sort((a, b) => {
    let rarityDiff = a.rarity - b.rarity; // Use descending order for rarity
