<%*
const folderPaths = [
    "Fallout-RPG/Items/Consumables/Chems",
    "Fallout-RPG/Items/Consumables/Other Consumables"
]; // Multiple folders

const excludedFolders = ["ExpiredChems", "IllicitSubstances"]; // Folders to exclude
const priorityFolders = ["RareChems", "MilitaryGrade"]; // Higher priority folders (only used if randomizeSelection = true)
    ...
    for (const file of files) {
        ...
        if (match) {
            ...
            for (const line of statblockContent) {
                // Improved regex to extract both quoted and unquoted values correctly
                const keyValueMatch = line.match(/^([\w\s]+):\s*(?:"([^"]+)"|([^\n]+))$/);
                if (keyValueMatch) {
                    const key = keyValueMatch[1].trim().toLowerCase(); // Normalize key
                    let value = keyValueMatch[2] ? keyValueMatch[2].trim() : keyValueMatch[3].trim();
            ...
            let name = parsedData["chem name"] || parsedData["name"] || "Unknown"; 
