...
function renderStatsSection() {	
	...
	restBtn.onclick = () => {
	    let lck = parseInt(document.getElementById("LCK")?.value) || 0;
	    ...
	    if (maxHpInput && currHpInput) {
	        ...
	        currHpInput.dispatchEvent(evt);
				    ...
				    // For Luck Points
		const luckNum = luckWrapper.querySelector('.plusminus-num'); // or a more precise selector if needed
		if (luckNum) luckNum.textContent = lck;
		
		// For Current HP
		const hpNum = hpWrapper.querySelector('.plusminus-num'); // adjust selector as needed
		if (hpNum && maxHpInput) hpNum.textContent = maxHpInput.value;
