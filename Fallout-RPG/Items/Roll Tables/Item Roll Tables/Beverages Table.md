...
const STORAGE_KEY = 'fallout_beverages_filter_inputs';
const TABLE_KEY = 'fallout_beverages_table_results';
    ...
    vault.getFiles()
        ...
        .map(async file => {
            ...
            if (statblockMatch) {
                ...
                return {
                    file,
                    name: statblock["consumables name"] || file.name,
...
const renderTable = (data) => {
    ...
    if (data.length === 0) {
        resultsContainer.textContent = "No beverages match the selected filters.";
...
button.addEventListener('click', () => {
    ...
    // ✅ Now use slicedPages instead of filteredPages
