...
const STORAGE_KEY = 'fallout_apparel_filter_inputs';
const TABLE_KEY = 'fallout_apparel_table_results';
...
const folderPath = "Fallout-RPG/Items/Apparel/";
...
const renderTable = (data) => {
    ...
    if (data.length === 0) {
        resultsContainer.textContent = "No apparel match the selected filters.";
        return;
    }
...
button.addEventListener('click', () => {
    ...
    // ✅ Slice the results to match maxItems before saving
    const slicedPages = filteredPages.slice(0, maxItems);

    // ✅ Save the sliced (corrected) results for persistence
    ...
    // ✅ Render the table after slicing and saving
    renderTable(slicedPages);
});
