...
function applyGainRemove(map, opLine) {
  // opLine variations you described, e.g.:
  // "Gain [[Piercing]] (1)"
  // "Remove [[Inaccurate]]"
  // "Gain [[Effect]] (2) some text"
  const raw = String(opLine ?? "").trim();

  const m = raw.match(/^(Gain|Remove)\s+\[\[([^\]]+)\]\](?:\s*\((\-?\d+)\))?(.*)$/i);
  if (!m) return;
  ...
  if (verb === "gain") {
    if (!existing) {
      map.set(key, { name, num, extra });
      return;
    }

    // If numeric provided and existing numeric exists -> compound.
    if (num != null) {
      const cur = (existing.num != null) ? existing.num : 0;
      existing.num = cur + num;
    }

    // Preserve any existing extra; only set extra if existing lacks it
  ...
  for (const a of addons) {
    ...
    for (const eff of effs) {
      ...
      if (name === "weapon qualities") {
        applyGainRemove(qualitiesMap, desc);
        continue;
      }
      if (name === "weapon damage effects") {
        applyGainRemove(dmgEffectsMap, desc);
        continue;
      }
      if (name === "effects") {
